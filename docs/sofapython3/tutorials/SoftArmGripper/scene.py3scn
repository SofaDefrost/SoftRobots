...
def addAllCablesArm(node):
    ...
    if params.Simulation.inverse:
        middleCable.addObject('CableActuator', name="cable", minForce=0, indices=list(range(size)), pullPoint=[0., 0., 0.])
    else:
        middleCable.addObject('CableConstraint', name="cable", indices=list(range(size)), pullPoint=[0., 0., 0.], valueType="displacement")
    middleCable.addObject('RigidMapping', rigidIndexPerPoint=[4 * i + 1 for i in range(size)], mapForces=False, mapMasses=False, applyRestPosition=True)
...
def addArm(node):
    ...
    arm.addObject('UniformMass', indices=[4 * i + 1 for i in range(size)], totalMass=0.003*size, showAxisSizeFactor=0.03)  # set mass of each piece that connect the ribs
...
def addFinger(node, fingerId, translation, rotation):
    ...
    finger.addObject('MeshVTKLoader', name='loader', filename=params.Simulation.path + '/mesh/finger.vtk', rotation=rotation, translation=translation, scale=1000)
    finger.addObject('MeshTopology', src='@loader', name='container')
    finger.addObject('MechanicalObject', name='dofs', template='Vec3')
    finger.addObject('UniformMass', totalMass=0.0219)
    finger.addObject('TetrahedronFEMForceField', template='Vec3', name='FEM', method='large', poissonRatio=0.3, youngModulus=1.8e6)
    ...
    visu[fingerId].addObject('MeshSTLLoader', name="loader", filename=params.Simulation.path + "/mesh/finger.stl", scale=1000)
    visu[fingerId].addObject('OglModel', src="@loader", filename=params.Simulation.path + "/mesh/finger.stl", color=[1, 1, 1, 1], rotation=rotation, translation=translation)
...
def createScene(rootnode):
    ...
    if params.Simulation.inverse:
        # The target will be represented by an orange sphere in the scene. You can select the target with the mouse (left click) while pressing the shift key on the keyboard.
        ...
        arm.addObject('PositionEffector', template="Rigid3", effectorGoal=target.dofs.position.getLinkPath(), indices=arm.nbDofs.value-4, useDirections=[1,1,1,0,0,0])
