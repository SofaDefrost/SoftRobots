...
class ServoArm(Sofa.Prefab):
    ...
    def init(self):
        self.addObject('MechanicalObject',
                               ...
                               translation2=[0, 25e-3, 0])

    def setRigidMapping(self,path):

        self.addObject('RigidRigidMapping',name='mapping', input=path, index=self.indexInput.value)

        visual = self.addChild(VisualModel(visualMeshPath='Data/SG90_servoarm.stl', translation=[0.,-25.0e-3, 0.],scale=[1.0e-3,1.0e-3,1.0e-3], color=[1., 1., 1., 0.75]))
...
class ActuatedArm(Sofa.Prefab):
    ...
    def init(self):
        ...
        self.servomotor.angleIn.value = -1.57
...
def createScene(rootNode):
    ...
    def myanimate(target, factor):
        target.angleIn.value = -1.57*math.cos(factor * 2 * math.pi)
