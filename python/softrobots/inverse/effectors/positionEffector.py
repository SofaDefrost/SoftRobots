def PositionEffector(attachedTo=None,
    ...
    """Creates and adds a position effector. Should be used in the context of the resolution of an inverse problem: find the actuation that leads to a desired position of the effector.
    ...
    effector = attachedTo.createChild(name)
    ...
    effector.createObject('MechanicalObject', template=template, position=position,
                        rotation=rotation, translation=translation, scale=uniformScale)

    # Create a PositionEffector object with a name.
    # the indices are referring to the MechanicalObject's positions.
    positionEffector = effector.createObject('PositionEffector', template=template,
